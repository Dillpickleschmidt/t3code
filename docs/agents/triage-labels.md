...
These labels live on `Dillpickleschmidt/t3code` (see `issue-tracker.md`). The fork's tracker started empty, so the labels are created on first use:
...
`gh label create` fails if the label already exists; add `--force` to make it idempotent.
